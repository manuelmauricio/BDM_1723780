...
require_once "php/conexion.php";
  ...
  // Check existence of id parameter before processing further
   ...
   //     mysqli_close($link);
    ...
    if(isset($_GET["section"]) && !empty(trim($_GET["section"]))){
        // Get URL parameter
        $s_id =  trim($_GET["section"]);
        
        // Prepare a select statement
        $sql = "CALL sp_seccion('Z', ?, 'null','null','null','null','null')";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "i", $param_id2);
...
<?php
  echo '<h2 style="color:  #2F435E; font-size: 300%; font-weight: 500;"> ' . $s_nombre . '</h2>
  </div>
  <hr class="titlehr" style="border:  3px solid ' . $s_color . ';">';
