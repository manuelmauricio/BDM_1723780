...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    // Validate name
    $input_currus = trim($_POST["idadminw"]);
        $currus = $input_currus;


    $input_currnew = trim($_POST["idwrw"]);
        $currnew = $input_currnew;




 $query = "CALL sp_sendnoticia('$currnew')";

                
if($stmt =  mysqli_query($link, $query)){
            
                header("location: ReporteroNoticias.php?user=$currus");
                exit();
            } else{
                echo "$currnew";
            }
...
<div id="main">
  <h2>¿Está seguro de que quiere enviar esta noticia al administrador ahora?</h2>
...
<div class="container">
  <div class="row">
    <div class="col-12">
  <form form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data" class="form-inline" role="form">

                        <input type="hidden" name="idwrw" value="<?php echo $idwr; ?>"/>
                         <input type="hidden" name="idadminw" value="<?php echo $iduserx; ?>"/>
...
?>  
  ...
  <p>
  <button class="btn btnbackend" type="submit"><i class="fa fa-send fa-fw"></i>&nbsp;&nbsp;Enviar noticia</button>
<?php      echo'<a href="ReporteroNoticias.php?user=' . $iduserx . '" class="btn btnbackend" type=""><i class="fa fa-close fa-fw"></i>&nbsp;&nbsp;Cancelar</a>'; ?>  
