...
$db_selectsection = 'S';
 ...
 $idnoticiax = $n_titulo = $n_lugar = $n_descripcion = $n_fecha =  $n_seccion_noticia =  $n_texto = $n_destacada  = $n_foto ="";
    if(isset($_GET["written"]) && !empty(trim($_GET["written"]))){
        // Get URL parameter
        $idnoticiax =  trim($_GET["written"]);
        
        // Prepare a select statement
        $sql = "CALL sp_editnoticia(?)";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "i", $param_idn);
            
            // Set parameters
            $param_idn = $idnoticiax;
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    $n_titulo = $row["titulo"];
                    $n_lugar = $row["lugar"];
                    $n_descripcion = $row["descripcion"];
                    $n_fecha = $row["fecha"];
                    $n_seccion_noticia = $row["seccion_noticia"];
                    $n_texto = $row["texto"];
                    $n_destacada = $row["destacada"];
                    $n_foto = $row["fotothumb"];
        mysqli_free_result($result);
                } else{
                echo "Oops! Something went wrong. Please try again later.";
                }
    ...
    $input_palabra = trim($_POST["palabra"]);
        ...
        if (isset($_FILES['photo']))
        {
            ...
            if (!isset($msg)) // If there was no error
            {
                ...
                //
                //mysql_query($query);
                $query = "CALL sp_updatenoticia('$noti', '$titulo', '$descripcion',  '$lugar', '$fecha', '$seccion', '$texto',
                '$destacada', '$data');";
...
<div id="main">
  ...
  <p>Edita esta noticia</p>

  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
  <div class="form-row">
    <div class="form-group col-md-12">
      <label for="inputName">Título:</label>
      <input value="<?php echo $n_titulo; ?>" name="titulo" id="titulo" pattern=".{6,}" type="name" pattern="[a-zA-Z0-9]{6,}" class="form-control"  aria-describedby="namelHelp" placeholder="Título de la noticia">
    </div>   
</div>
  ...
  <div class="form-row">
    <div class="form-group col-md-12">
      ...
      <input value="<?php echo $n_descripcion; ?>" name="descripcion" id="descripcion" pattern=".{6,}" type="name" pattern="[a-zA-Z0-9]{6,}" class="form-control" aria-describedby="namelHelp" placeholder="Descripción de la noticia">
    </div>   
</div>
...
<div class="form-row">
    <div class="form-group col-md-4">
      ...
      <input value="<?php echo $n_lugar; ?>" name="lugar" id="lugar" pattern=".{6,}" type="user" pattern="[a-zA-Z0-9]{6,}" class="form-control" aria-describedby="userHelp" placeholder="Lugar: Colonia, Ciudad, País
">
    </div>
  
    <div class="form-group col-md-4">
      <label for="inputPassword">Fecha:</label>
      <input value="<?php echo $n_fecha; ?>" name="fecha" id="fecha" pattern=".{6,}" type="text" pattern="[a-zA-Z0-9]{6,}" class="form-control" aria-describedby="PasswordHelp" placeholder="Fecha-Hora del acontecimiento">
    </div>

    <div class="form-group col-md-4">
      <label for="inputName2">Sección:</label>
    <select value="<?php echo $n_seccion_noticia; ?>" class="form-control" name="seccion" id="seccion">
<?php
...
?>
  ...
  <div class="form-row">
   ...
   <div class="form-group col-md-12">
    ...
    <textarea name="texto" id="texto" pattern=".{6,}" class="form-control" rows="13"><?php echo $n_texto; ?></textarea>
  </div>
 ...
 <div class="form-group col-md-6">
      ...
      <input name="palabra" id="palabra"  class="form-control"  aria-describedby="nameHelp" placeholder="Palabra,ejemplo,etc">
...
<div class="form-group col-md-6">
    ...
    <select value="<?php echo $n_destacada; ?>" class="form-control" name="destacada" id="destacada"">
...
';?>
     ...
     <?php      echo '
<input name="noti" id="noti" type="hidden" value="' . $idnoticiax . '">
